...
def _run_dayzero(loop, cloud_name):
    cloud_hosts = quads.filter_hosts({"cloud": cloud_name})
    hosts = [h.name for h in cloud_hosts]
    if not hosts:
        return
    ...
    try:
        loop.run_until_complete(dispatcher.run_dayzero_cloud(hosts, cloud_name))
    except Exception as e:
    ...
    with Progress(
        ...
        for ass in _assignments:
            ...
            if not ass.notification.initial:
                ...
                _run_dayzero(loop, ass.cloud.name)
