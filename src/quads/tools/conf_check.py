...
from quads.config import QUADS_CONF_DIR
...
def run_conf_check(conf_dir=None):
    if conf_dir is None:
        conf_dir = QUADS_CONF_DIR
...
def main(logger=None):
    ...
    table = Table(title="Configuration Check Results", show_header=True, header_style="bold cyan")
    ...
    logger.info(f"Files checked: {result.files_checked}, Errors: {error_count}, Warnings: {warn_count}")
    return 1
