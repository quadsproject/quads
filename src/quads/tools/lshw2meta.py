    ...
    with Progress(
        ...
        for filename in json_files:
            with open(filename) as _f:
                    ...
                    if child.value.get("vendor"):
                        for host_interface in host_obj.interfaces:
                            if host_interface.mac_address == child.value["serial"]:
                                ...
                                rows.append(
                                    ("interface", f"{host_interface.name}  {host_interface.mac_address}", "updated")
                                )
                # disks
                ...
                for child in [
                    child for child in children if child.value["class"] == "memory" and "bank" not in child.value["id"]
                ]:
                ...
                for child in [child for child in children if child.value["class"] == "processor"]:
                    ...
                    if configuration.get("cores") and configuration.get("threads"):
                        ...
                        rows.append(
                            ("cpu", f"{d['vendor']}  {d['product']}  {d['cores']}c/{d['threads']}t", "created")
                        )
                # GPU
                ...
                if progress.console.is_terminal:
                    table = Table(
                        title=f"[bold]{hostname}[/]", show_header=True, header_style="bold cyan", min_width=60
                    )
