...
class Foreman(object):
    ...
    async def get_os_id(self, os_type):
        endpoint = f"/operatingsystems/{os_type}"
        result = await self.get(endpoint)
    ...
    async def _select_os_resource(self, fetch_fn, os_id, resource, os_type, host_name):
        items = await fetch_fn(os_id)
        if not items:
            logger.error(f"No {resource} found for OS '{os_type}' (id={os_id})")
            return None
        selected = items[0]
        if len(items) > 1:
            logger.warning(
                f"OS '{os_type}' has {len(items)} {resource}, "
                f"defaulting to first: '{selected['name']}'. "
                f"Available: {[i['name'] for i in items]}"
            )
        else:
            logger.info(f"Selected {resource} '{selected['name']}' for OS '{os_type}' on {host_name}")
        return selected
    ...
    async def prepare_host_provisioning(self, host_name: str, cloud: str, os_type: str) -> bool:
        ...
        try:
            os_id = await self.get_os_id(os_type)
            if not os_id:
