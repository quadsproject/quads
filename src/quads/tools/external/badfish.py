...
class Badfish:
    ...
    async def get_boot_devices(self):
        if not self.boot_devices:
            ...
            if "Attributes" in data:
                ...
                if not boot_seq and not self.boot_mode_readable:
                    # The BIOS BootMode read failed, so get_boot_seq() assumed
                    # the legacy Bios BootSeq, which is empty on a UEFI host.
                    # Only in that case fall back to the populated sequence; a
                    # genuine Bios host must not be retargeted to UefiBootSeq.
    ...
    async def get_job_queue(self):
        ...
        try:
            ...
            logger.debug("Job collection not supported by this host.")
            return []
    ...
    async def patch_boot_seq(self):
        ...
        for _ in range(self.retries):
            if _status_code not in (200, 202, 204):
        ...
        if _status_code in (200, 202, 204):
        ...
        for i in range(self.retries):
            ...
            if status_code in [200, 202]:
                ...
                if status_code == 503 and i + 1 != self.retries:
                    ...
                    continue
                elif status_code == 400 and insist and i + 1 != self.retries:
                    await self.clear_job_queue()
                    if not _first_reset:
