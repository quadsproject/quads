...
def get_running_loop() -> asyncio.AbstractEventLoop:
    """
    Retrieves the currently running loop, or raises RuntimeError
    if called from a context where the loop is not running.
    """
    try:
        loop = asyncio.get_event_loop()
        if not loop.is_running():
            raise RuntimeError("No running event loop found. Ensure this is called within an async function.")
        return loop
    except RuntimeError:
        raise RuntimeError("No running event loop found. Ensure this is called within an async function.")
