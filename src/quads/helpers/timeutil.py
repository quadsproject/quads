...
* database column defaults use ``func.now()``
...
def format_http_date(dt):
    ...
    the actual instant.  This is what the REST API emits.
    """
    return ensure_utc(dt).strftime(HTTP_DATE_FORMAT)
...
def parse_http_date(value):
    ...
    if isinstance(value, datetime):
        return value
    return datetime.strptime(str(value), HTTP_DATE_FORMAT).replace(tzinfo=timezone.utc)
...
def parse_datetime(value):
    ...
    * ISO 8601 strings with or without a zone offset
    * plain ``%Y-%m-%d %H:%M`` / ``%Y-%m-%dT%H:%M`` strings (local wall clock)
    ...
    for fmt in ("%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M", "%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M"):
        try:
