...
The scheme is only correct when the application server and the database session
run in the same timezone; ``flask --app quads.server.app check-timezones``
surfaces a mismatch, and the schema migration shipped with this change re-anchors
rows that were generated by the database (``func.now()``) in the old format.
