...
class Host(Serialize, TimestampMixin, Base):
    ...
    uloc = Column(String)
    blade = Column(String)
    bootmode = Column(Enum("Bios", "Uefi", name="bootmode_enum"), default=None, nullable=True)
...
class Schedule(Serialize, TimestampMixin, Base):
    ...
    move_status = Column(
        Enum(MoveStatus, name="move_status_enum", create_type=False, values_callable=lambda e: [s.value for s in e]),
        nullable=True,
    )
