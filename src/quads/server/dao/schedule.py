...
class ScheduleDao(BaseDao):
    ...
    @classmethod
    def update_move_status(cls, schedule_id: int, **kwargs) -> Schedule:
        schedule = cls.get_schedule(schedule_id)
        if not schedule:
            raise EntryNotFound
        allowed = {"move_status", "move_message", "move_error"}
        for key, value in kwargs.items():
            if key in allowed:
                setattr(schedule, key, value)
        status = kwargs.get("move_status")
    ...
    def start_move_batch(cls, hostnames: list) -> dict:
        ...
        for hostname in hostnames:
            ...
            if not host:
                continue
            schedules = cls.get_current_schedule(host=host, date=now)
            if not schedules:
                continue
            schedule = schedules[0]
            if schedule.move_status and schedule.move_status not in ("completed", "failed"):
                schedule.move_status = "failed"
                schedule.move_error = "Superseded by new move"
                schedule.build_end = now
