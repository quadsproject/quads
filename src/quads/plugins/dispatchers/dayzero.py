...
class DayzeroDispatcher(MultiPluginDispatcher[DayzeroPlugin]):
    ...
    async def run_dayzero(
        ...
        if not plugins:
            return {}
        return await self._dispatch_plugins(plugins, host, cloud)
    ...
    async def run_dayzero_cloud(
        ...
        if not plugins:
            return {}
        return await self._dispatch_plugins(plugins, hosts, cloud)

    async def _dispatch_plugins(self, plugins, host, cloud) -> Dict[str, bool]:
        tasks = []
        plugin_names = []
        for plugin in plugins:
            tasks.append(self._execute_plugin(plugin, host, cloud))
    ...
    async def _execute_plugin(self, plugin, host, cloud) -> bool:
        dayzero_logger = logging.getLogger("quads.plugins.dayzero")
        label = host if isinstance(host, str) else f"{cloud} ({len(host)} hosts)"
        try:
            dayzero_logger.info(f"Running dayzero plugin {plugin.name} on {label}")
            success = await plugin.execute(host=host, cloud=cloud)
            if success:
