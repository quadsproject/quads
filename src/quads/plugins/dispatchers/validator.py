...
class ValidatorDispatcher(SinglePluginDispatcher[ValidatorPlugin]):
    ...
    async def validate(
        ...
        try:
            ...
            return await plugin.validate(cloud, assignment, hosts, skip_system, skip_network, report)
...
async def validate(
    cloud: str, assignment: Assignment, hosts: List, skip_system: bool, skip_network: bool, report: str = ""
) -> Tuple[bool, str]:
    dispatcher = get_validator_dispatcher()
    return await dispatcher.validate(cloud, assignment, hosts, skip_system, skip_network, report)
