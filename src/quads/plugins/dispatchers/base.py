...
class BaseDispatcher(Generic[T]):
    ...
    def _get_plugin_class(self) -> Optional[Type[T]]:
        """
        Get the plugin class (not instance) to instantiate.

        This allows dispatchers to create new plugin instances with custom arguments.

        Returns:
            The plugin class to instantiate, or None if no plugin is enabled
        """
