...
class HardwareDispatcher(BaseDispatcher[HardwarePlugin]):
    """
    ...
    async def reboot_server(self, graceful: bool = False) -> bool:
        ...
        logger.info(f"Performing {reboot_type} reboot via {self._default_plugin.name}")
        try:
...
def get_hardware_dispatcher(plugin_manager: Optional[PluginManager] = None) -> HardwareDispatcher:
    """Get the global HardwareDispatcher instance"""
