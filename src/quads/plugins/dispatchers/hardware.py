...
class HardwareDispatcher(SinglePluginDispatcher[HardwarePlugin]):
    ...
    async def init(self, host: str, rack: str, uloc: str, blade: str) -> bool:
        ...
        try:
            ...
            plugin_config = self.plugin_manager.config.get("plugins", {}).get(
                self._default_plugin.name if self._default_plugin else "badfish", {}
            )
            self._runtime_plugin = plugin_class(plugin_config)
            if not self._runtime_plugin.initialize(self.plugin_manager):
                logger.error(f"Failed to initialize {plugin_class.__name__}")
                self._runtime_plugin = None
                return False
            await self._runtime_plugin.init(host, rack, uloc, blade)
            return True
