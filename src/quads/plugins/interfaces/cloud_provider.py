...
Allows QUADS to provision resources from cloud providers (AWS, IBM Cloud, Azure, etc.)
when local bare metal capacity is exhausted.
"""
...
class CloudProviderPlugin(BasePlugin):
    """
    ...
    async def validate_quota(self, instance_type: str, count: int = 1) -> bool:
        """
        Check if quota allows provisioning requested instances.

        Args:
            instance_type: Instance type to provision
