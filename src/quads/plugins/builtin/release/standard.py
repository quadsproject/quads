...
class StandardReleasePlugin(ReleasePlugin):
    ...
    async def _report_progress(self, progress_id, status, message="", error_message=""):
        if not progress_id:
            return
        try:
            ...
            await asyncio.to_thread(self.quads.update_move_progress, progress_id, data)
    ...
    async def move_and_rebuild(
        ...
        await self._report_progress(progress_id, "ipmi_config", "IPMI credentials configured")
        ...
        if rebuild and target_cloud.name != host_obj.default_cloud.name:
            ...
            if _is_supermicro:
                ...
                await self._report_progress(progress_id, "provisioning", "Preparing provisioner")
                if not await self.provisioner_dispatcher.prepare_host_provisioning(host, new_cloud, os_type):
                    self._update_host_on_failure(host_obj, progress_id=progress_id)
                    return False

                await self._report_progress(progress_id, "reboot", "PXE boot and power cycle")
                ...
                async with semaphore:
                    ...
                    if not ok:
                        self._update_host_on_failure(host_obj, progress_id=progress_id)
                        return False
                    await self._report_progress(progress_id, "hardware_prep", "Hardware prepared")

                    if not await self.power_on_host(host_obj):
                        self.logger.error(f"Failed to power on {host}")
                        self._update_host_on_failure(host_obj, progress_id=progress_id)
                        return False
                    await self._report_progress(progress_id, "power_on", "Host powered on")
                    ...
                    if not await self.provisioner_dispatcher.prepare_host_provisioning(host, new_cloud, os_type):
                        self._update_host_on_failure(host_obj, progress_id=progress_id)
                        return False
                    await self._report_progress(progress_id, "provisioning", "Provisioner ready")

                    await self.cleanup_virtual_media(host_obj)
                    await self._report_progress(progress_id, "cleanup", "Virtual media cleaned")
                    ...
                    if vendor == "Dell":
                        ...
                        if not await ipmi.pxe_persistent():
                            ...
                            self._update_host_on_failure(host_obj, progress_id=progress_id)
                            return False
                    await self._report_progress(progress_id, "reboot", "Rebooting for rebuild")
        ...
        await self._report_progress(progress_id, "post_install", "Records updated")
        return True
    ...
    def _update_host_on_failure(self, host_obj, progress_id=None) -> None:
