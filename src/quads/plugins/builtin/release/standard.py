...
class StandardReleasePlugin(ReleasePlugin):
    ...
    async def prepare_host_provisioning(
        ...
        try:
            ...
            if not os_id:
                self.logger.error(f"OS type {os_type} not found in Foreman")
                return False
            ...
            for result in foreman_results:
                if isinstance(result, Exception) or not result:
                    self.logger.error("There was something wrong setting Foreman host parameters.")
                    return False

            return True
        except Exception as ex:
            self.logger.error(f"Error setting up Foreman for {host}: {ex}")
            return False
