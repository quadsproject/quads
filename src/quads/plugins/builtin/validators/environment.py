    ...
    async def verify_hardware_creds(self, host, password):
        ...
        try:
            ...
            if await self.hardware_dispatcher.init_for_host(
                ...
                self.hardware_dispatcher._runtime_plugin.username = str(Config["ipmi_cloud_username"])
                self.hardware_dispatcher._runtime_plugin.password = password
                # Re-initialize with new credentials
                await self.hardware_dispatcher._runtime_plugin.init()
        except Exception:
