...
class EnvironmentValidatorPlugin(ValidatorPlugin):
            ...
            if new_ips:
                ...
                if not result:
                    ...
                    for ip in hosts_set:
                        ...
                        self.logger.warning("%s on %s (%s)", ip, hostname, iface)
                    return False, report
    ...
    def _get_progress_map(self, hosts):
        ...
        for host in hosts:
            try:
                progress = self.quads.get_move_status(host.name)
                if progress:
    ...
    def _update_progress(self, schedule_id, status, message=""):
        if not schedule_id:
            return
        try:
            data = {"status": status, "message": message}
            self.quads.update_move_status(schedule_id, data)
        except Exception:
            pass
