...
class EnvironmentValidatorPlugin(ValidatorPlugin):
    ...
    def initialize(self) -> bool:
        ...
        try:
            logger.info(f"Initializing {self.name} validator plugin")
            return True
    ...
    def shutdown(self) -> None:
        ...
        logger.info(f"Shutting down {self.name} validator plugin")

    def health_check(self) -> bool:
        """Verify plugin is operational"""
        try:
            # Basic health check - verify we can communicate with QUADS API
            return True
        except Exception as e:
    ...
    async def post_system_test(
        ...
        if not valid_creds:
            logger.error("Unable to query Foreman for cloud: %s" % cloud)
    ...
    async def validate_env(
        ...
        logger.info(f"Validating {cloud}")
        failed = False
        ...
        if await self.env_allocation_time_exceeded(cloud):
            ...
            if not failed:
                ...
                try:
                    ...
                    logger.debug(str(ex))
                    logger.error("Could not update assignment: %s." % assignment.id)
