...
class CloudCmdPlugin(DayzeroPlugin):
    ...
    async def execute(self, host, cloud) -> bool:
        ...
        if not host:
            self.logger.info(f"No hosts in {cloud}, skipping")
            return True

        first_host = sorted(host)[0]
    ...
    async def _ssh_exec(self, host, command) -> bool:
        ...
        try:
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(), timeout=SSH_TIMEOUT
            )
        ...
        if proc.returncode != 0:
            self.logger.error(
                f"SSH to {host} failed (rc={proc.returncode}): {stderr.decode().strip()}"
            )
            return False
