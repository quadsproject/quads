...
class CloudCmdPlugin(DayzeroPlugin):
    ...
    async def execute(self, cloud) -> bool:
        ...
        schedules = quads.get_schedules({"assignment_id": assignment.id})
        if not schedules:
            self.logger.info(f"No schedules for {cloud}, skipping")
            return True

        first_host = schedules[0].host.name
        self.logger.info(f"Running release command for {owner} on {first_host} in {cloud}")

        encoded = base64.b64encode(command.encode("utf-8")).decode("ascii")
        tmux_cmd = f"tmux new-session -d -s quads_release " f"'echo {encoded} | base64 -d | bash'"
