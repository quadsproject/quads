...
class AristaSwitchPlugin(SwitchPlugin):
    ...
    async def configure(self, host: str, old_cloud: str, new_cloud: str) -> bool:
        ...
        if not _host_obj.interfaces:
            ...
            return False
        all_interfaces = sorted(_host_obj.interfaces, key=lambda k: k.name)
        interfaces = [
            i for i in all_interfaces if not getattr(i, "switch_vendor", None) or i.switch_vendor == self.name
        ]
        if not interfaces:
            return True
        self.logger.debug("Connecting to switch on: %s" % interfaces[0].switch_ip)
        switch_ip = None
        ssh_helper = None
        for i, interface in enumerate(interfaces):
            last_nic = interface.name == all_interfaces[-1].name
            if not switch_ip:
    ...
    async def ls_config(self, cloud, all=False):
        ...
        for host in hosts:
            ...
            if host and host.interfaces:
                all_interfaces = sorted(host.interfaces, key=lambda k: k.name)
                interfaces = [
                    i for i in all_interfaces if not getattr(i, "switch_vendor", None) or i.switch_vendor == self.name
                ]
