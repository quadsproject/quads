...
class AWSCloudProvider(CloudProviderPlugin):
    """
    ...
    name = "aws_cloud"
    version = "1.0.0"
    description = "AWS EC2 cloud provider for overflow capacity"
    author = "QUADS Team"
    ...
    async def get_available_capacity(self, instance_type: Optional[str] = None, region: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.debug(f"AWS available capacity: {available} (max: {max_instances})")

            return available

        except Exception as e:
            logger.error(f"Failed to get AWS capacity: {e}")
            return 0
    ...
    async def create_instance(
        ...
        **kwargs,
        ...
        try:
            ...
            if not ami_id:
                logger.error("No AMI specified and no default configured")
                return None
