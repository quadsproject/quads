...
class ProvisionerDispatcher(BaseDispatcher[ProvisionerPlugin]):
    ...
    async def delete_host(self, hostname: str) -> bool:
        """Delete host from provisioner backend"""
        if not self._default_plugin:
            logger.error("No provisioner plugin enabled")
            return False

        logger.info(f"Deleting {hostname} from {self._default_plugin.name}")
        try:
            return await self._default_plugin.delete_host(hostname)
        except Exception as e:
