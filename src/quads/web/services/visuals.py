...
class VisualsService:
    ...
    def get_metadata(self, when: str) -> Dict[str, Any]:
        """
        Get fast metadata for immediate UI feedback using optimized database queries.
        ...
        # Use optimized database query for all metrics
        metrics = ScheduleDao.get_allocation_metrics(first_day, last_day)
        ...
        result = {
            ...
            "quick_metrics": {
                "daily_utilization": metrics["daily_utilization"],
                "systems_in_use": metrics["current_allocations"],
                "total_systems": metrics["total_hosts"],
                "monthly_utilization": metrics["monthly_utilization"],
            },
            "loading_config": {
                "total_hosts": metrics["total_hosts"],
                "recommended_chunk_size": min(100, max(20, metrics["total_hosts"] // 5)),
                "estimated_chunks": (metrics["total_hosts"] + 99) // 100,  # Ceiling division for 100-host chunks
                "estimated_load_time": min(30, (metrics["total_hosts"] // 100) * 2),  # Rough estimate in seconds
            },
        }
    ...
    def get_hosts_summary(self, when: str) -> Dict[str, Any]:
        """
        Get host list and summary data for table structure using optimized queries.
        ...
        # Use optimized database query for host priority info
        hosts_data = ScheduleDao.get_hosts_with_allocation_priority(
            first_day, last_day, priority="mixed", offset=0, limit=10000  # Get all hosts
        )

        # Get allocation metrics for summary
        metrics = ScheduleDao.get_allocation_metrics(first_day, last_day)

        return {
            "hosts": [
                {"hostname": host["hostname"], "has_schedules": host["has_allocations"]}
                for host in hosts_data["hosts"]
            ],
            "summary": {
                "total_hosts": hosts_data["batch_info"]["total_hosts"],
                "days_in_month": days_in_month,
                "hosts_with_schedules": metrics["allocated_hosts"],
                "estimated_monthly_utilization": metrics["monthly_utilization"],
            },
        }

    def get_host_batch(self, when: str, offset: int = 0, limit: int = 100, priority: str = "mixed") -> Dict[str, Any]:
        """
        Get a batch of host allocation data for progressive loading using optimized queries.
        ...
        try:
            # Get hosts with priority ordering using optimized query
            hosts_batch_info = ScheduleDao.get_hosts_with_allocation_priority(
                first_day, last_day, priority=priority, offset=offset, limit=limit
            )

            if not hosts_batch_info["hosts"]:
                return self._empty_batch_response(offset, limit, priority)

            # Get detailed daily allocation data for this batch
            host_names = [host["hostname"] for host in hosts_batch_info["hosts"]]
            daily_data = ScheduleDao.get_daily_allocation_data(
                first_day, last_day, host_names=host_names, offset=0, limit=limit
            )

            # Process the daily data and apply color/emoji assignments
            host_data = []
            batch_allocated = 0

            for daily_host in daily_data:
                hostname = daily_host["hostname"]
                days_data = []
                allocated_count = 0

                for day_info in daily_host["days"]:
                    day_data = {
                        "day": day_info["day"],
                        "cloud": day_info["cloud"],
                        "description": day_info["description"],
                        "owner": day_info["owner"],
                        "ticket": day_info["ticket"],
                    }

                    if day_info["allocated"] and day_info["assignment_id"]:
                        # Apply color and emoji assignment
                        if day_info["assignment_id"] not in self.assignment_colors:
                            self.assignment_colors[day_info["assignment_id"]] = len(self.assignment_colors) + 1

                        chosen_color_idx = self.assignment_colors[day_info["assignment_id"]]
                        day_data.update(
                            {
                                "color": self.colors[chosen_color_idx],
                                "emoji": f"&#{self.emojis[chosen_color_idx]};",
                        ...
                        day_data.update(
                            {
                                "color": self.colors[0],  # Default gray
                                "emoji": None,
                            }
                        )
            ...
            return {
                "batch_info": hosts_batch_info["batch_info"],
                "hosts": host_data,
                "batch_stats": {
                    "allocated_days_in_batch": batch_allocated,
                    "utilization_in_batch": (
                        (batch_allocated * 100) // (len(host_data) * days_in_month) if host_data else 0
                    ),
                },
            }

        except Exception as e:
            print(f"Error in get_host_batch: {e}")
            return self._empty_batch_response(offset, limit, priority, error=str(e))
