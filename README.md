...
| run_mode | Behavior |
|----------|----------|
| `per_cloud` (default) | `execute()` is called once per cloud with a list of hostnames and schedule dicts |
| `per_host` | `execute()` is called once per host with a single hostname and schedule dict |
...
3. Set `name` to match your filename (without `.py`) and optionally set `run_mode` (`per_cloud` is the default)
4. Add a config entry in `plugins.yml` with `enabled: true`

**per_cloud example** (default, called once for the entire cloud):
...
class SummaryPlugin(DayzeroPlugin):
    ...
    async def execute(self, host, cloud, schedule_data):
        # host is a list of hostname strings
        # schedule_data is a list of dicts
...
class MyPlugin(DayzeroPlugin):
    ...
    async def execute(self, host, cloud, schedule_data):
        # host is a single hostname string
