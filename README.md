...
QUADS is designed to handle large amounts of hosts and independent, isolated multi-tenant environments in a single namespace and L2 network.  QUADS is limited only by the VLAN limit in the (Q-in-Q) 802.1Q IEEE spec which specifies **4090** VLAN per namespace.
   ...
   - For example, if the maximum number of QUADS-managed internal interfaces I had in one of my hosts was 4, QUADS can support 1,021 individual environments containing allocated servers in multi-tenant networks with a combined host count of [24,540 servers.](https://x.com/i/grok/share/OzKXShaLzPiZazD46dzyJPZvo) or `98,160 vmembers / 4` per instance.
