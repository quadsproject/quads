...
plugins/builtin/dayzero/
    runonce/
        moveinfo.py          # shipped example plugin (per_host)
...
class MyPlugin(DayzeroPlugin):
    name = "myplugin"
    version = "1.0.0"
    description = "Example per-host contrib plugin"
    author = "Your Name"
    run_mode = "per_host"
    ...
    async def execute(self, host, cloud, schedule_data):
        ...
        # schedule_data is a single dict with stage_timestamps
...
class SummaryPlugin(DayzeroPlugin):
    name = "summary"
    version = "1.0.0"
    description = "Example per-cloud contrib plugin"
    author = "Your Name"
    run_mode = "per_cloud"
