...
>
> In move and rebuild, Badfish is used to check the current bootmode for a host and compared against the host bootmode setting. When a host's current bootmode does not match the defined bootmode for the host, badfish is used to ensure correct setting. It is recommended to set this parameter for managed hosts to avoid provisioning problems.
