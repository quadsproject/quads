...
* There is no limit to the amount of past, present or future assignments a cloud can have.
...
### What is a Schedule
* A `schedule` is host-level metadata for each QUADS-managed host that tells it what `cloud` it is part of, where it should move to or from if needed and is referenced by it's `schedule-id`.
* One to many hosts with the same schedule with any overlap of start/end date(s) comprise a QUADS environment or workload.
* Hosts store a lifetime history of all clouds they were part of for the duration of their service.
