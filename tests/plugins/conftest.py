...
class MockDayzeroPlugin(DayzeroPlugin):
    """Mock per-host dayzero plugin for testing"""

    name = "mock_dayzero"
    version = "1.0.0"
    description = "Mock per-host dayzero plugin for testing"
    author = "Test"
    run_mode = "per_host"
    ...
    async def execute(self, host: str, cloud: str) -> bool:
        return True
...
class MockDayzeroCloudPlugin(DayzeroPlugin):
    ...
    async def execute(self, host: list, cloud: str) -> bool:
        return True


class FailingDayzeroCloudPlugin(DayzeroPlugin):
    """Per-cloud dayzero plugin that raises during execute"""

    name = "failing_dayzero_cloud"
    version = "1.0.0"
    description = "Per-cloud dayzero plugin that fails"
    author = "Test"
    run_mode = "per_cloud"
    ...
    async def execute(self, host: list, cloud: str) -> bool:
        raise RuntimeError("Simulated per-cloud dayzero failure")


class FailingDayzeroPlugin(DayzeroPlugin):
    """Per-host dayzero plugin that raises during execute"""

    name = "failing_dayzero"
    version = "1.0.0"
    description = "Per-host dayzero plugin that fails"
    author = "Test"
    run_mode = "per_host"
