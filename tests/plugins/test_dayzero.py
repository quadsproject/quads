...
from tests.plugins.conftest import MockDayzeroPlugin, MockDayzeroCloudPlugin, FailingDayzeroPlugin
...
class TestDayzeroInterface:
    ...
    def test_dayzero_plugin_complete_implementation(self):
        ...
        result = asyncio.run(
            plugin.execute("host01.example.com", "cloud02", {"stage_timestamps": {}})
        )
...
class TestDayzeroDispatcher:
    ...
    def test_run_dayzero_success(self, mock_handler, setup_manager_with_dayzero):
        ...
        result = asyncio.run(
            dispatcher.run_dayzero("host01.example.com", "cloud02", schedule_data)
        )
    ...
    def test_run_dayzero_no_plugins(self, mock_handler):
        ...
        result = asyncio.run(
            dispatcher.run_dayzero("host01.example.com", "cloud02", {})
        )
        assert result == {}
    ...
    def test_run_dayzero_cloud_success(self, mock_handler):
        ...
        result = asyncio.run(
            dispatcher.run_dayzero_cloud(hosts, "cloud02", schedule_data_list)
        )
    ...
    def test_run_dayzero_cloud_no_plugins(self, mock_handler):
        ...
        dispatcher = DayzeroDispatcher(manager)
        result = asyncio.run(
            dispatcher.run_dayzero_cloud(["host01.example.com"], "cloud02", [{}])
        )
        assert result == {}
...
class TestMoveInfoPlugin:
    ...
    def test_execute_success(self, mock_subprocess):
        ...
        result = asyncio.run(
            plugin.execute("host01.example.com", "cloud02", schedule_data)
        )
        assert result is True
