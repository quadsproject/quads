...
class TestCreateSchedule:
    ...
    @pytest.mark.parametrize("prefill", prefill_self_schedule, indirect=True)
    def test_ssm_model_limit_patch_no_dates(self, test_client, auth, prefill, monkeypatch):
        """
        | GIVEN: A self-schedule on a one-host model
        | WHEN: Its build window (not start/end) is patched, then an empty PATCH
        | THEN: The build-only patch succeeds and the empty one is a 400
        """
        monkeypatch.setattr(Config, "ssm_model_limit", {"6018R": 50}, raising=False)
        auth_header = auth.get_auth_header()
        _create_host(test_client, auth_header, "host1003.example.com", model="6018r")
        req = SELF_SCHEDULE_1_REQUEST.copy()
        req["hostname"] = "host1003.example.com"
        response = unwrap_json(test_client.post("/api/v3/schedules", json=req, headers=auth_header))
        assert response.status_code == 201
        schedule_id = response.json["id"]
        ...
        response = unwrap_json(
            test_client.patch(
                f"/api/v3/schedules/{schedule_id}",
                json={"build_start": "2080-07-01T12:00"},
                headers=auth_header,
            )
        )
        assert response.status_code == 200
