...
async def test_move_and_rebuild_updates_bootmode(test_client, auth, prefill, mock_dependencies, caplog):
    ...
    assert mock_dependencies["badfish"].get_bios_attribute.call_args.args[0] == host_name
    assert mock_dependencies["badfish"].get_bios_attribute.call_args.args[1] == "BootMode"
    assert (
        f"Current bootmode (Uefi) does not match default bootmode Bios for {host_name}. Setting it now." in caplog.text
    )
