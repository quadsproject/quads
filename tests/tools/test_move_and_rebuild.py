...
def mock_dependencies():
    ...
    # We need to mock dependencies in BOTH modules:
    # 1. move_and_rebuild.py (for move_and_rebuild)
    # 2. helpers/utils.py (for check_assignment_provisioning_status)

    # We Create shared mocks so we can assert on them regardless of source
    shared_update_assign = MagicMock()
    shared_get_assign = MagicMock()
    ...
    with patch("quads.tools.move_and_rebuild.badfish_factory", new_callable=AsyncMock) as mock_bf, \
         ...
         patch("quads.tools.move_and_rebuild.quads.update_assignment", shared_update_assign), \
         patch("quads.tools.move_and_rebuild.quads.get_active_cloud_assignment", shared_get_assign), \
         patch("quads.tools.move_and_rebuild.quads.get_current_schedules", shared_get_schedules), \
         patch("quads.helpers.utils.quads.update_assignment", shared_update_assign), \
         patch("quads.helpers.utils.quads.get_active_cloud_assignment", shared_get_assign), \
         patch("quads.helpers.utils.quads.get_current_schedules", shared_get_schedules), \
        ...
        yield {
            "update_assignment": shared_update_assign,
            "get_assignment": shared_get_assign,
...
async def test_move_and_rebuild_success_does_not_update_assignment(test_client, auth, prefill, mock_dependencies):
    # Tests that move_and_rebuild focuses ONLY on the host
    ...
    # Assignment update should NOT be called here anymore (it was moved to orchestration)
...
async def test_check_assignment_provisioning_status_success(test_client, auth, prefill, mock_dependencies):
    # Tests the batch processing logic independently
    target_cloud = "cloud02"
