...
def mock_dependencies():
    ...
    # Mock BaseDao.safe_commit to verify commits
    mock_safe_commit = MagicMock()

    # We patch quads.server.app.quads methods because that is the object instance
    # used by the code under test (both in move_and_rebuild and utils)
    with patch("quads.tools.move_and_rebuild.badfish_factory", new_callable=AsyncMock) as mock_bf, \
         ...
         patch("quads.server.app.quads.update_host", side_effect=side_effect_update_host), \
         patch("quads.server.app.quads.update_schedule"), \
         patch("quads.server.app.quads.get_active_cloud_assignment", shared_get_assign), \
         patch("quads.server.app.quads.get_current_schedules", shared_get_schedules), \
...
async def test_move_and_rebuild_success_does_not_update_assignment(test_client, auth, prefill, mock_dependencies):
    ...
    # This checks that we didn't commit an assignment change
