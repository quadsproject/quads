...
def test_get_authenticated_user_sends_bearer_only(monkeypatch):
    ...
    def fake_send(self, request, **kwargs):
        ...
        response._content = b'{"email": "alice@example.com", "roles": ["user"]}'
        response.headers["Content-Type"] = "application/json"
        return response

    monkeypatch.setattr(requests.Session, "send", fake_send)
    user = QuadsApi(Config).get_authenticated_user("qat_testtoken")

    assert user == {"email": "alice@example.com", "roles": ["user"]}
    assert captured["url"].endswith("/me")
    assert captured["auth_header"] == "Bearer qat_testtoken"
