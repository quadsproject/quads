...
class TestCloudResourceReserved:
    def test_reserved_math_uses_utc_reanchored_local_time(self):
        """The second re-anchored CLI callsite (action_cloudresource) must keep
        the 48h lock math correct for an active assignment."""
        with force_tz(OS_TZ):
            cli, quads = _make_cli()
            ...
            cloud = MagicMock(spec=Cloud)
            cloud.name = "free01"
            cloud.last_redefined = wire_time
            assignment = MagicMock()

            with (
                patch.object(type(quads), "get_cloud", return_value=cloud),
                patch.object(type(quads), "get_active_cloud_assignment", return_value=assignment),
            ):
                exit_code = cli.run(action="cloudresource", cli_args={"cloud": "free01"})

            assert exit_code == 1
            warnings = [c[0][0] for c in cli.logger.warning.call_args_list if c[0]]
            reserved = [m for m in warnings if "(reserved:" in m]
            assert reserved, f"expected a reserved warning, got: {warnings}"
            match = re.search(r"\(reserved: ([0-9]+)hr ([0-9]+)min", reserved[0])
            assert match, reserved[0]
            total_hours = int(match.group(1)) + int(match.group(2)) / 60
            # 48h lock minus roughly 1h elapsed.  A double shift (treating the
            # UTC value as local) would give ~51h and a sign error ~43h.
            assert 46 < total_hours < 48, total_hours
