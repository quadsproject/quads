...
and legacy token login. Access requires a Google SSO identity (browser
session or `qat_` API token) or the legacy time-limited token from
`POST /api/v3/login/`. Only the cloud owner, a cc-user of the cloud, or an
admin can download them; anyone else gets HTTP 403.
