...
* The database server and the application server must run in the same timezone: rows
  written by the application (all `created_at` defaults, `last_redefined`, `last_login`,
  `last_used`) are stored as application-local wall clock. Run
  `flask --app quads.server.app check-timezones` after deploying to verify, and run
  `flask --app quads.server.app db upgrade` at deploy time so the shipped migration
  re-anchors historical rows written by the database session in the old format.
