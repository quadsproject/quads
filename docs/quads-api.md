...
* The following construct can be used via http ```POST``` to receive more detailed data by providing granular criteria to return JSON body data:
  ...
  * Valid POST URI queries
    ...
    - ```/api/v3/moves/progress/```    Create a move progress record (admin)
    - ```/api/v3/moves/progress/batch```  Batch-create move progress records (admin)
  * Valid PATCH URI queries
    - ```/api/v3/moves/progress/<id>```  Update move progress status (admin)
