...
* Behavior notes: a model set to 0% is never self-schedulable. A non-zero percentage
  ...
  below one for a non-zero percentage (`max(1, floor(pct/100 * fleet))`): it can
  undershoot the percentage by up to one host, and for fleets too small to yield a
  whole host at that percentage it exceeds it (e.g. 10% of a 3-host fleet allows 1 host,
  i.e. 33%). Admin batch schedules on self-service clouds are capped too, so a batch
  that exceeds the remaining pool is rejected as a whole.
