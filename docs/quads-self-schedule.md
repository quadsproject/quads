...
> what can still be grabbed. A model at 0% is never self-scheduleable; 100% (the default)
...
* Behavior notes: a model set to 0% is never self-scheduleable. A non-zero percentage
  always reserves at least one slot, so small fleets are never silently disabled; the
  cap is `max(1, floor(pct/100 * fleet))`, which matches exactly the advertised
  percentage for any fleet that yields at least one host. Admin batch schedules on
  self-service clouds are capped too, so a batch that exceeds the remaining pool is
  rejected as a whole.
