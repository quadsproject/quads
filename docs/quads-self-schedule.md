...
* Behavior notes: a model set to 0% is never self-scheduleable; models with
  `floor(pct/100 * fleet) == 0` (tiny fleets below 100/pct hosts) are disabled as well;
  admin batch schedules on self-service clouds are capped too, so a batch that exceeds
  the remaining pool is rejected as a whole.
