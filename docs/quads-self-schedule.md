...
> Start date is now and end date is whatever is set in your `conf/selfservice.yml` [configuration](https://github.com/redhat-performance/quads/blob/latest/conf/selfservice.yml#L6) for `ssm_default_lifetime`.
...
> [!NOTE]
> The `assignment id` is the one returned in the assignment creation response e.g. `"description":"Testing Assignment","id":110`
...
curl -k \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  https://quads.example.com/api/v3/assignments/terminate/{assignment_id}
```
...
> [!NOTE]
> If using the [quads-lib](https://python-quads-lib.readthedocs.io/en/latest/usage.html) with the context manager, you don't need to call the login method and the logout method will be called automatically when the context manager is exits.
